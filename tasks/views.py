...
class delTask(APIView):
    ...
    def post(self, request):
        ...
        return Response(data={"code": 200, "message": "Success!"}, status=HTTP_200_OK)
...
class addAIM(APIView):
    ...
    def post(self, request):
        ...
        AIModel.objects.create(
            ai_name=request.data["ai_name"],
            ai_url=request.data["ai_url"],
            ai_status=request.data["ai_status"],
            ai_description=request.data["ai_description"],
            ai_type=request.data["ai_type"],
            ai_credit=request.data["ai_credit"],
        )
        return Response(data={"code": 200, "message": "Success!"}, status=HTTP_200_OK)
...
class validate(APIView):
    ...
    def post(self, request):
        ...
        if ai_instance.ai_credit > request.user.credit:
            return Response(data={"code": 100}, status=HTTP_200_OK)

        return Response(data={"code": 200}, status=HTTP_200_OK)
...
class numTask(APIView):
    ...
    def post(self, request):
        Task_num = Task.objects.filter(is_delete=0, user_id_id=request.user.id).aggregate(Task_num=Count("task_id"))
        Task_finish = Task.objects.filter(status=100, is_delete=0, user_id_id=request.user.id).aggregate(
            Task_finish=Count("task_id")
        )
        ...
        return Response(
            data={
                "code": 200,
                "data": {
                    "num_of_task": str(Task_num["Task_num"]),
                    "num_of_finished_tasks": str(Task_finish["Task_finish"]),
                },
            },
            status=HTTP_200_OK,
        )
...
class prediction(APIView):
    ...
    def post(self, request):
        ...
        Transaction.objects.create(
            user_id=UserProfile.objects.get(id=model_data.user_id_id),
            status=1,
            method="income",
            order=model_data.ai_name,
            credit=round(model_data.ai_credit / 2),
        )
...
class updatemodelImage(APIView):
    ...
    def post(self, request):
        ...
        uuid_namespace = uuid.uuid3(uuid.NAMESPACE_OID, str(UserProfile.objects.get(id=request.user.id).id))
        uuid_str = str(uuid.uuid3(uuid_namespace, str(uuid.uuid4()))) + ".jpg"
...
class trainingMaterialCount(APIView):
    ...
    def post(self, request):
        ...
        amount.save()
        return Response(data={"code": 200, "message": "AImodel updated."}, status=HTTP_200_OK)
...
class updateAIM(APIView):
    ...
    def post(self, request):
        ...
        AIM.save()
        return Response(data={"code": 200, "message": "bingo!"}, status=HTTP_200_OK)
