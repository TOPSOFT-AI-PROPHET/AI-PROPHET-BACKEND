...
class newTask(APIView):
    ...
    def post(self, request):
        ...
        Task.objects.create(user_id_id=user_id.id,
                            ai_id_id=ai_id, description=description)
        return Response(
...
class delTask(APIView):
    ...
    def post(self, request):
        Task.objects.filter(user_id=request.user,
                            task_id=request.data["task_id"]).update(is_delete=1)
        return Response(
...
class details(APIView):
    ...
    def post(self, request):
        ...
        for i in range(sourcedata["total_param"]):
            ...
            ai_params.append(
                {"para_name": sourcedata["details"][i]["name"], "para_value": ai_json[i][str(i)]})
